    ...
    def identify_structures(self):
        ...
        for root_node in range(self.n_points):
            if not visited[root_node]:
                current_tree_points = []
                # DFSUtil appends to the list passed, and returns it.
                self.DFSUtil(current_tree_points, root_node, visited)
                
                # Check if the discovered component is too small (e.g., isolated points)
                # The original check was len(tree_points) <= 1, which is fine.
                # We ensure all points in such a small tree are marked for removal if desired.
                if len(current_tree_points) <= 1:
                    for point_in_small_tree in current_tree_points:
                        if point_in_small_tree not in self.removed_nodes: # Avoid duplicates
                            self.removed_nodes.append(point_in_small_tree)
                    continue
                trees.append(current_tree_points)
        self.n_limbs = len(trees)

        all_rods_list = self.rods.tolist() # self.rods is populated based on np.inf in connectivity_mat
        
        structures = []
        for tree_points in trees: # tree_points is a list of point indices in one connected component
            current_limb_rods = []
            for rod in all_rods_list: # Iterate over all originally identified rods
                p1, p2 = rod
                # A rod belongs to this limb's structure if both its endpoints are in this tree
                if p1 in tree_points and p2 in tree_points:
                    current_limb_rods.append(rod)
            
            # Add this structure. It's possible current_limb_rods is empty
            # if the tree is just a single point (e.g., a hip connected to base but no further segments),
            # though the len(current_tree_points) <= 1 check should handle truly isolated points.
            structures.append([current_limb_rods, tree_points])
